    ...
    <>
      ...
      <main style={{ marginTop: '4rem', padding: '2rem 1rem', backgroundColor: '#f9fafb', minHeight: '100vh' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <h1 style={{ textAlign: 'center', fontSize: 'clamp(2rem, 5vw, 3rem)', marginBottom: '2rem' }}>Compare Rewards for Top Travel Cards</h1>
...
export default function RewardsCompareCalculator() {
  const [spending, setSpending] = useState({ travel: 0, dining: 0, groceries: 0, gas: 0, other: 0 });
  ...
  const calculateRewards = (card) => {
    if (!card) return null;
    const rewards = { travel: 0, dining: 0, groceries: 0, gas: 0, other: 0, totalPoints: 0, estimatedValue: 0, breakdown: {} };
    ...
    const cpp = card.redemptionOptions?.cash_back_cpp || 1.0;
          ...
          <div key={index} className={styles.dropdownGroup}>
            <select value={card || ""} onChange={(e) => handleCardSelect(index, e.target.value)}>
              ...
              {cardsData.cards.map((c) => (
                <option key={c["Card Name"]} value={c["Card Name"]}>{c["Card Name"]}</option>
              ))}
            </select>
          </div>
        ))}
        ...
        <div className={styles.tableWrapper}>
          <table className={styles.rewardsTable}>
            <thead>
              <tr>
                <th>Category</th>
                {selectedCards.map((card, index) => card && (<th key={index}>{card}</th>))}
              </tr>
            </thead>
            <tbody>
              ...
              <tr>
                ...
                {selectedCards.map((cardName, index) => {
                  const card = cardsData.cards.find(c => c["Card Name"] === cardName);
                  const rewards = calculateRewards(card);
                  return card ? (<td key={index}>${rewards.estimatedValue.toFixed(2)}</td>) : (<td key={index}>-</td>);
                })}
              </tr>
            </tbody>
          </table>
