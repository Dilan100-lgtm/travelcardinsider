...
// --- Define Interfaces ---
// (Interfaces: CardCap, CardReward, CardSignUpBonus, CardRedemptionOptions, CardPerk, DetailedCreditCard - remain the same)
...
// Helper function to get the "Other" multiplier for a card
const getOtherMultiplier = (card: DetailedCreditCard): number => {
  ...
  // (useMemo hook remains the same as the previous version with bonus/perk integration)
  ...
  // (getAiRecommendation function remains the same)
   ...
   // Helper function for basic AI output formatting (remains the same)
  ...
  // --- Render JSX ---
  return (
    <div style={{ /* ... CSS variables ... */ } as React.CSSProperties}>
      <h2 style={{ textAlign: 'center', marginBottom: '1.5rem', color: 'var(--label-color)' }}>
          AI-Powered Travel Rewards Calculator
      </h2>

       {/* Controls Section */}
       <div style={{ /* ... styling ... */ display: 'flex', justifyContent: 'center', gap: '1.5rem', marginBottom: '1.5rem', padding: '1rem', backgroundColor: 'var(--light-bg)', borderRadius: '8px', flexWrap: 'wrap', maxWidth: '600px', margin: '0 auto 2rem auto' }}>
            <div>
      ...
      {/* Spending Input Form */}
      <form style={{ /* ... styling ... */ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem', marginBottom: '2.5rem', maxWidth: '900px', margin: '0 auto 2.5rem auto' }}>
        {categoryList.map((category) => ( /* ... input rendering ... */ <div key={category} style={{ display: 'flex', flexDirection: 'column' }}> <label htmlFor={category} style={{ marginBottom: '0.5rem', textTransform: 'capitalize', fontWeight: '500', color: 'var(--label-color)', fontSize: '0.95em' }}> {category === 'onlineShopping' ? 'Online Shopping (US)' : category.replace(/([A-Z])/g, ' $1')} (Monthly $): </label> <input type="number" id={category} name={category} value={spend[category as keyof SpendInput]} onChange={handleChange} min={0} step={10} placeholder="$0" style={{ padding: '0.8rem', border: '1px solid var(--input-border-color)', borderRadius: '4px', fontSize: '1rem', backgroundColor: 'var(--input-bg-color)', }} /> </div> ))}
      </form>
      ...
      <div style={{ marginTop: '2rem' }}>
        <h3 style={{ textAlign: 'center', marginBottom: '1.5rem', color: 'var(--label-color)' }}>
         ...
         {/* Add Compare Button if there are enough results */}
       ...
       {showComparison && cardsToCompare.length > 0 && (
          ...
          <div style={{
              ...
              <div style={{
                  ...
                  <div style={{
                      display: 'grid',
                      // 4 columns: Labels + 3 Cards
                      gridTemplateColumns: 'minmax(150px, 1fr) repeat(3, minmax(150px, 2fr))',
                      ...
                      {cardsToCompare.map(card => (
                          <div key={card["Card Name"]} style={{...compareCellStyle(true, false), textAlign: 'center'}}>
                              <img src={card.image || 'placeholder.png'} alt={card["Card Name"]} style={{ height: '40px', objectFit: 'contain', marginBottom: '0.3rem' }}/>
                      ...
                      {renderCompareRow("Bonus", cardsToCompare.map(c => `${c.signUpBonus?.description || 'N/A'} (~$${c.signUpBonus?.estimated_value_usd ?? 0})`))}
                      {renderCompareRow("Top Reward Rates", cardsToCompare.map(c => (
                          <ul style={{margin: 0, paddingLeft: '1em', textAlign: 'left', fontSize: '0.9em'}}>
                              {(Array.isArray(c.rewards) ? c.rewards.filter(r => r.multiplier > 1 && r.category !== 'other').sort((a,b) => b.multiplier - a.multiplier).slice(0, 3) : [])
                               ...
                               <li>{getOtherMultiplier(c)}x Other</li>
                          </ul>
                      )))}
                       {renderCompareRow("Key Perks", cardsToCompare.map(c => (
                           <ul style={{margin: 0, paddingLeft: '1em', textAlign: 'left', fontSize: '0.9em'}}>
                               {(Array.isArray(c.perks) ? c.perks.filter(p => ['lounge_access', 'free_checked_bag', 'travel_credit', 'companion_fare', 'companion_certificate', 'global_entry_tsa_precheck_credit', 'annual_hotel_credit', 'anniversary_points', 'anniversary_miles', 'hilton_status'].includes(p.type) || p.value_usd > 0).slice(0, 4) : [])
                                   .map(p => <li key={p.type+(p.description || '')}>{formatPerkForCompare(p)}</li>)}
                           </ul>
                       )))}
                      {renderCompareRow("Redemption Strategy Used", cardsToCompare.map(c => redemptionStrategy))}
                      ...
                      {renderCompareRow("Links", cardsToCompare.map(c => (
                          <>
                              <a href={c.reviewLink} target="_blank" rel="noopener noreferrer" style={{ display: 'block', marginBottom: '0.3rem' }}>Review</a>
                              <a href={c.applyLink} target="_blank" rel="noopener noreferrer" style={{ display: 'block' }}>Apply</a>
                          </>
                      )))}
