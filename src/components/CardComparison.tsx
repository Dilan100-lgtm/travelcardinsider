...
export default function CardComparison({ cards, initiallySelectedCards = [] }: CardComparisonProps) {
  ...
  useEffect(() => {
    const newSelection: (CreditCard | null)[] = [null, null, null];
    initiallySelectedCards.slice(0, 3).forEach((card, index) => {
                ...
                {selected.map((card, i) => (
                  <td key={i}>
                    {card ? (
                      type === 'image' ? (
                        <Image
                          src={(card as any)['image'] || '/placeholder-card.png'}
